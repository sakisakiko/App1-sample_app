...
describe '投稿のテスト' do
  ...
  describe 'トップ画面(top_path)のテスト' do
    before do
    # itの処理をする前にこの処理をする（itを行う前の前提条件）
      visit top_path
      # visit 指定したパスへの画面遷移
    end
    context '表示の確認' do
      it 'トップ画面(top_path)に「ここはTOPページです」が表示されているか' do
        expect(page).to hava_content 'ここはTOPページです'
      end
  ...
  describe '投稿画面のテスト' do
    ...
    context '投稿処理後のテスト' do
     it '投稿処理後のリダイレクト先は正しいか' do
      ...
      # have_current_path(パスを取得できる)　詳細画面に遷移。（投稿したものなのでidは最新のものになる（List.last)）
     end
    end
  end
  ...
  describe '編集画面のテスト' do
    ...
    context '表示の確認' do
      it '編集前のタイトルと本文がフォームに表示（セット）されている' do
        expect(page).to have_feild 'list[title]',with: list.title
        expect(page).to have_feild 'list[body]',with: list.body
      end
      it '保存ボタンが表示される' do
    ...
    context '更新処理に関するテスト' do
      it '更新後のリダイレクト先は正しいか' do
       ...
       fill_in 'list[body]',with: Faker::Lorem.characters(number:30)
       click_button '保存'
       expect(page).to have_currernt_path list_path(list)
      end
    end
  end
end
